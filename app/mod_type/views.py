...
def electionType(datasource, typeizbori,year, instanca, krug):
    ...
    if typeizbori != "predsjednicki":
        ...
        if year == 2000:
            ...
            urlterritories = "http://datacentar.io/api/izbori/2/parlamentarni/godina/" + str(year) + "/teritorija/instanca/3"
    else:
    ...
    urlturnout = "http://datacentar.io/api/izbori/datasource/2/" + str(typeizbori) + "/godina/" + str(year)+"/instanca/2"
    ...
    if typeizbori=="parlamentarni":
        if year==2000:
            url_winner_territory = "http://datacentar.io/api/izbori/1/"+str(typeizbori)+"/godina/"+str(year)+"/teritorija/instanca/3/krug/None"
        elif year in [2007,2016]:
            url_winner_territory = "http://datacentar.io/api/izbori/winners/1/" + str(typeizbori) + "/godina/" + str(year) + "/instanca/3/krug/None"
        else:
            url_winner_territory = "http://datacentar.io/api/izbori/winners/2/" + str(typeizbori) + "/godina/" + str(year)+"/instanca/3/krug/None"
        response_winner = urllib.urlopen(url_winner_territory)
    if typeizbori=="predsjednicki":
        url_winner_territory = "http://datacentar.io/api/izbori/winners/2/" + str(typeizbori) + "/godina/" + str(year) + "/instanca/3/krug/"+str(krug)
        response_winner = urllib.urlopen(url_winner_territory)

    for winner in json_util.loads(response_winner.read()):
        winner_territory.append(winner)

    return render_template('type/type.html',typeizbori=typeizbori,year=year,data=data,territories=territories,results_voters=results_voters,list_political_parties=list_political_parties,winner_territory=winner_territory,krug=krug)
